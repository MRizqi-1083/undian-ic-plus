...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('plugins/undian/css/style23.css') }}">
    ...
    <style>
        ...
        #scene {
            ...
            margin-top: -300px;
            margin-left: -600px;
            background: url(/undian/234567.jpg) no-repeat center center fixed;
...
********** */
 ...
 s */
        ...
        .modal-body {
            background: url(undian/grandprize.png) no-repeat no-repeat center;
...
<body>
    <div id='starter' onclick="showTime()"><img class="logo" src="{{ asset('undian/logo23.png') }}"
            style="width:150px"></img></div>
    <div id='scene'>
        <div id='curtain'>
            <h1><img src="{{ asset('undian/volta.png') }}" class="wuling" alt="Wuling" style="width:18%;">
            </h1>
                        ...
                        <div id="planeMachine">
                            @foreach ($dtnya as $k => $v)
                                <p id="{{ $k }}" pid="{{ $v->peserta_id }}" phone="{{ $v->phone }}"
                                    mail="{{ $v->email }}" nama="{{ strtoupper($v->full_name) }}">
                                    {{ strtoupper($v->full_name) }}
                                </p>
                            @endforeach
                        </div>
                    </div>
    ...
    <div class="modal fade" id="winnerModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"
        ...
        @csrf
        <div class="modal-dialog modal-dialog-centered modal" style="overflow:hidden;min-width:700px">
            <div class="modal-content modalWinner" style="background-color:#fff0">
                <div class="modal-body ">
                    ...
                    <div class="row text-center">
                        <div class="col-12 wwx">
                            <span class="menang" style="font-size:22px;margin-top:5px;"></span>
                            <br>
        ...
        const listriqu = new SlotMachine(planeMachine, {
            ...
            onComplete: function(z) {
                setTimeout(function() {
                    ...
                    $("span.nomor").html($('#planeMachine p[id="' + z + '"]').attr("phone"));
                    $("span.detil").html($('#planeMachine p[id="' + z + '"]').attr("mail"));
                }, 1000);
